...
public class ExcelReportView extends AbstractXlsView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		try {
			response.setHeader("Content-Disposition", "attachment; filename=\"ExcelBook.csv\"");

			List<LibraryBean> list = (List<LibraryBean>) model.get("excelReport");
			//List<LibraryBean> list = bookdao.getExcel();
...
//			header.createCell(1).setCellValue("UserName");
			...
			System.out.println("records are:User Id:\n" + records.getUserId() + "records are:User Name:\n" + records.getUserName() + "Book ID : \n" + 
							records.getBookId() + "Book Name:"+ records.getBookName() + "Book Status: " + records.getBookStatus());
				Row datarow = sheet.createRow(row_count++);
//				datarow.createCell(0).setCellValue(records.getUserId());
//				datarow.createCell(1).setCellValue(records.getUserName());
